var m4 = {
  ...
  transpose: function(pMatrix) {
    return [
      ...
      pMatrix[4], pMatrix[7], pMatrix[11], pMatrix[15],
    ]
  },

};

function getVectorNormals(positions){
  const n = positions.length;
  var vNormals = [];
  for (var i = 0; i < n; i = i + 4*3){
    ...
    const v1 = subtractVectors(B, A);
    const v2 = subtractVectors(C, A);
    const normalDir = cross(v1, v2);
    const vNormal = normalize(normalDir);
    for (var j = 0; j < 4; j++){
      vNormals = vNormals.concat(vNormal);
    }
  }
  return vNormals;
}
...
var ui = {
  setupSlider: function setupSlider(selector, options) {
    ...
    return ui.createSlider(parent, options); // eslint-disable-line
  },

  createSlider: function createSlider(parent, options) {
    ...
    var name = params["ui-" + options.name] || options.name;
...
function save() {
  ...
  var file = new Blob([JSON.stringify(model)], {type : 'application/json'});
